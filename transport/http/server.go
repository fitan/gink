...
func (s Server) ServeHTTP(gCtx *gin.Context) {
	ctx := gCtx.Request.Context()

	if len(s.finalizer) > 0 {
		iw := &interceptingWriter{gCtx.Writer, http.StatusOK, 0}
		defer func() {
	...
	request, err := s.dec(ctx, gCtx)
	if err != nil {
		s.errorHandler.Handle(ctx, err)
		s.errorEncoder(ctx, err, gCtx)
		return
	}
	...
	if err := s.enc(ctx, gCtx, response); err != nil {
		s.errorHandler.Handle(ctx, err)
		s.errorEncoder(ctx, err, gCtx)
		return
	}
...
type ErrorEncoder func(ctx context.Context, err error, gCtx *gin.Context)
...
type ServerFinalizerFunc func(ctx context.Context, code int, gCtx *gin.Context)
...
func NopRequestDecoder(ctx context.Context, context2 *gin.Context) (interface{}, error) {
	return nil, nil
}
...
func DefaultErrorEncoder(_ context.Context, err error, gCtx *gin.Context) {
